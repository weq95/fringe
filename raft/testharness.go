...
func NewHarness(t *testing.T, n int) *Harness {
	...
	for i := 0; i < n; i++ {
		...
		ns[i] = NewServer(i, peerIds, commitChans[i])
		ns[i].Serve()
		alive[i] = true
	}
...
// Shutdown 集群所有机器停机
// shuts down all the servers in the harness and waits for them to
// stop running.
...
// DisconnectPeer 断开对等点【关闭连接】
// disconnects a server from all other servers in the cluster.
func (h *Harness) DisconnectPeer(id int) {
	tlog("Disconnect[关闭连接] %d", id)
...
// ReconnectPeer 重新连接对等点[重新加入集群]
// connects a server to all other servers in the cluster.
func (h *Harness) ReconnectPeer(id int) {
	tlog("Reconnect[重连] %d", id)
	h.cluster[id].raft.state = Follower
	//go h.cluster[id].Serve()
	for j := 0; j < h.n; j++ {
		if j != id && h.alive[j] {
...
// CrashPeer 崩溃点
// "crashes" a server by disconnecting it from all peers and then
...
// RestartPeer 重启对等点 【重启服务】
// "restarts" a server by creating a new Server instance and giving
// it the appropriate storage, reconnecting it to peers.
func (h *Harness) RestartPeer(id int) {
	if h.alive[id] {
		log.Fatalf("id=%d is alive in RestartPeer[服务器正常，重启]", id)
	}
	tlog("Restart %d[服务重启]", id)
	...
	ready := make(chan interface{})
	h.cluster[id] = NewServer(id, peerIds, h.commitChans[id])
	h.cluster[id].Serve()
	h.ReconnectPeer(id)
	close(ready)
	h.alive[id] = true
...
// CheckSingleLeader 检查单一领导者
// checks that only a single server thinks it's the leader.
...
func (h *Harness) CheckSingleLeader() (int, int) {
			...
			if h.connected[i] {
				...
				if isLeader {
					if leaderId < 0 {
						...
						h.t.Fatalf("both %d and %d think they're leaders[多个领导者]", leaderId, i)
					}
				}
			}
		}
	...
	h.t.Fatalf("leader not found[没有找到领导者]")
	return -1, -1
}

// CheckNoLeader 检查无领导者
// checks that no connected server considers itself the leader.
func (h *Harness) CheckNoLeader() {
	for i := 0; i < h.n; i++ {
		if h.connected[i] {
			_, _, isLeader := h.cluster[i].raft.Report()
			if isLeader {
				h.t.Fatalf("server %d leader; want none[服务领导者]", i)
			}
		}
	}
}
...
// CheckCommitted 检查提交
// verifies that all connected servers have cmd committed with
...
func (h *Harness) CheckCommitted(cmd int) (int, int) {
	h.mu.Lock()
	defer h.mu.Unlock()
		...
		if h.connected[i] {
			if commitsLen >= 0 {
				...
				if len(h.commits[i]) != commitsLen {
					h.t.Fatalf("commits[%d] = %d, commitsLen = %d[数据同步不一致]", i, h.commits[i], commitsLen)
				}
			} else {
...
// CheckCommittedN 检查已提交N
// verifies that cmd was committed by exactly n connected
// servers.
...
// CheckNotCommitted 检查未提交
// verifies that no command equal to cmd has been committed
...
func (h *Harness) CheckNotCommitted(cmd int) {
	...
	for i := 0; i < h.n; i++ {
		...
		for c := 0; c < len(h.commits[i]); c++ {
			...
			if gotCmd == cmd {
				h.t.Errorf("found %d at commits[%d][%d], expected none [预计没有]", cmd, i, c)
			}
		}
	}
}

// SubmitToServer 提交到服务器
// submits the command to serverId.
func (h *Harness) SubmitToServer(serverId int, cmd interface{}) bool {
	return h.cluster[serverId].raft.Submit(cmd)
}
...
// 收集提交
// reads channel commitChans[i] and adds all received entries
...
func (h *Harness) collectCommits(i int) {
	for c := range h.commitChans[i] {
		h.mu.Lock()
		tlog("collectCommits(机器id：%d) got 数据： %+v", i, c)
